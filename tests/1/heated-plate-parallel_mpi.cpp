...
void Matrix::init_communication(const int rank, const int numTasks) {
  ...
  MPI_Send_init(data[1], this->columns, MPI_DOUBLE, other, tag_to_use,
                MPI_COMM_WORLD, &comm_requests[0]);
  MPI_Recv_init(data[0], this->columns, MPI_DOUBLE, other, tag_to_use,
  ...
  MPI_Send_init(data[this->rows], this->columns, MPI_DOUBLE, other, tag_to_use,
...
std::pair<int, double> calculate(int rank, double epsilon, Matrix &matrix_in) {
  // int i, j;
  ...
  while (epsilon <= diff) {
    ...
    diff = run_iteration(temp_matrix, matrix_in.data, rows, columns,
                         size_of_block);
    // in theory, it is possible to start sending the halo lines around right
    // after they are computed, but this will mess up the current implementation
    // with cache blocking
...
#define EPSILON 0.005
  ...
  }
  {
    ...
    auto [iterations, diff] = calculate(rank, epsilon, mIn);

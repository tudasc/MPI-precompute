...
void Matrix::init_communication(const int rank, const int numTasks,const int msg_tag) {
...
double
run_iteration(double **Matrix_In, double **Matrix_Out, const int rows, const int columns, const int size_of_block) {

    double diff = 0;
    const int number_of_blocks =
            (columns % size_of_block) == 0 ? columns / size_of_block : 1 + (columns / size_of_block);
...
std::pair<int, double> calculate(const int rank, const double epsilon, Matrix &matrix_in,Matrix &matrix_out) {
    // int i, j;
...
#pragma GCC unroll 2
    ...
    while (epsilon <= diff) {
        ...
        if (iterations % 2 == 0) // even
        {
            ...
            diff = run_iteration(matrix_out.data, matrix_in.data, rows, columns, size_of_block);
            ...
            diff = run_iteration(matrix_in.data, matrix_out.data, rows, columns, size_of_block);
...
#define EPSILON 0.005
    ...
    }
    {
        ...
        auto [iterations, diff] =
                calculate(rank, epsilon, mIn,mOut);

...
template <typename T> void SimulationNBodyV1Vectors<T>::init() {
  this->flopsPerIte =
...
template <typename T> void SimulationNBodyV1Vectors<T>::initIteration() {
        ...
        for (unsigned short iVecPos = 0; iVecPos < mipp::N<T>(); iVecPos++) {
          ...
          for (unsigned short jVecPos = 0; jVecPos < mipp::N<T>(); jVecPos++) {
            ...
            SimulationNBodyV1<T>::computeAccelerationBetweenTwoBodies(
                this->G, positionsX[iBody], positionsY[iBody],

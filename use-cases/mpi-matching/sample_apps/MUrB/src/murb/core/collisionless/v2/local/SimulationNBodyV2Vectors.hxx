...
template <typename T> SimulationNBodyV2Vectors<T>::~SimulationNBodyV2Vectors() {
...
template <typename T> void SimulationNBodyV2Vectors<T>::reAllocateBuffers() {
  if (this->nMaxThreads > 1) {
    ...
    this->allocatedBytes +=
        (this->bodies->getN() + this->bodies->getPadding()) * sizeof(T) *
        (this->nMaxThreads - 1) * 3;
  }

  this->flopsPerIte =
      25.f * ((float)this->bodies->getN() * 0.5f) * (float)this->bodies->getN();
}

template <typename T> void SimulationNBodyV2Vectors<T>::initIteration() {
  for (unsigned long iBody = 0;
       iBody < this->bodies->getN() * this->nMaxThreads; iBody++) {
...
#pragma omp parallel
  {
    ...
    const unsigned long tStride =
        tid * (this->bodies->getN() + this->bodies->getPadding());
...
#pragma omp for schedule(runtime)
    for (unsigned long iVec = 0; iVec < this->bodies->getNVecs(); iVec++) {
      ...
      for (unsigned short iVecPos = 0; iVecPos < mipp::N<T>(); iVecPos++) {
        ...
        for (unsigned short jVecPos = iVecPos + 1; jVecPos < mipp::N<T>();
             jVecPos++) {
          const unsigned long jBody = jVecPos + iVecOff;
          SimulationNBodyV2<T>::computeAccelerationBetweenTwoBodies(
              this->G, masses[iBody], positionsX[iBody], positionsY[iBody],
              positionsZ[iBody], this->accelerations.x[iBody + tStride],
              this->accelerations.y[iBody + tStride],
              this->accelerations.z[iBody + tStride],
              this->closestNeighborDist[iBody], masses[jBody],
              positionsX[jBody], positionsY[jBody], positionsZ[jBody],
      ...
      for (unsigned long jVec = iVec + 1; jVec < this->bodies->getNVecs();
           jVec++)
        for (unsigned short iVecPos = 0; iVecPos < mipp::N<T>(); iVecPos++) {
          const unsigned long iBody = iVecPos + iVecOff;
    ...
    for (unsigned long iBody = 0; iBody < this->bodies->getN(); iBody++)
      for (unsigned iThread = 1; iThread < this->nMaxThreads; iThread++) {
        this->accelerations.x[iBody] +=
            this->accelerations
                .x[iBody + iThread * (this->bodies->getN() +
                                      this->bodies->getPadding())];
        this->accelerations.y[iBody] +=
            this->accelerations
                .y[iBody + iThread * (this->bodies->getN() +
                                      this->bodies->getPadding())];
        this->accelerations.z[iBody] +=
            this->accelerations
                .z[iBody + iThread * (this->bodies->getN() +
                                      this->bodies->getPadding())];
      }
}

...
template <typename T> SimulationNBodyV2<T>::~SimulationNBodyV2() {
...
template <typename T> void SimulationNBodyV2<T>::reAllocateBuffers() {
...
template <typename T> void SimulationNBodyV2<T>::initIteration() {
...
#pragma omp for schedule(runtime)
    for (unsigned long iBody = 0; iBody < this->bodies->getN(); iBody++)
      for (unsigned long jBody = iBody + 1; jBody < this->bodies->getN();
           jBody++)
    ...
    for (unsigned long iBody = 0; iBody < this->bodies->getN(); iBody++)
      for (unsigned iThread = 1; iThread < this->nMaxThreads; iThread++) {
        this->accelerations.x[iBody] +=
            this->accelerations.x[iBody + iThread * this->bodies->getN()];
        this->accelerations.y[iBody] +=
            this->accelerations.y[iBody + iThread * this->bodies->getN()];
        this->accelerations.z[iBody] +=
            this->accelerations.z[iBody + iThread * this->bodies->getN()];
      }
}

// 25 flops
template <typename T>
void SimulationNBodyV2<T>::computeAccelerationBetweenTwoBodies(
  ...
  // compute the acceleration value between body j and body i: || aj || = G.mi /
  // || rij ||^3

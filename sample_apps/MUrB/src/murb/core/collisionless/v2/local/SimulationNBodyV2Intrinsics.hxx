                                                                               ...
                                                                               mipp::Reg<T> &rclosNeighj)
{
	...
	auto raTmp = rG / (rrij * rrijSquared); // 2 flops

	auto rai = raTmp * rmj; // 1 flop

	raiX = mipp::fmadd(rai, rrijX, raiX); // 2 flops
	raiY = mipp::fmadd(rai, rrijY, raiY); // 2 flops
	raiZ = mipp::fmadd(rai, rrijZ, raiZ); // 2 flops

	auto raj = raTmp * rmi; // 1 flop

	rajX = mipp::fnmadd(raj, rrijX, rajX); // 2 flops
	rajY = mipp::fnmadd(raj, rrijY, rajY); // 2 flops
	rajZ = mipp::fnmadd(raj, rrijZ, rajZ); // 2 flops

	rclosNeighi = mipp::min(rrij, rclosNeighi);
	rclosNeighj = mipp::min(rrij, rclosNeighj); // TODO: this second minimum is not threads safe !!!
}
                                                                                   ...
                                                                                   mipp::Reg<float> &rclosNeighj)
{
	...
	auto raTmp = rG * rrijInv * rrijInv * rrijInv; // 3 flops
	...
	rclosNeighj = mipp::max(rrijInv, rclosNeighj); // TODO: this second minimum is not threads safe !!!
}

...
void create_indexed_data(MPI_Datatype* dtype, int size) {
    ...
    for(int i = 0; i < INDEXED_NUM_BLOCKS; ++i) {
        disps[i] = blocksize * i;
        lengths[i] = (int)ceil(((double)i / (double)INDEXED_NUM_BLOCKS) * (double)blocksize);
    }
...
void use_one_sided_persistent(MPI_Datatype* dtype, int count, int size, int num_iters, MPI_Info* info) {
    MPIOPT_INIT();
    ...
    if (rank == 1) {
        ...
        MPIOPT_Send_init_x(buffer, count, *dtype, 0, 42, MPI_COMM_WORLD, &req, *info);
        ...
        MPIOPT_Recv_init_x(buffer, count, *dtype, 1, 42, MPI_COMM_WORLD, &req, *info);
...
void use_standard_comm(MPI_Datatype* dtype, int count, int size, int num_iters) {
    int rank, numtasks;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &numtasks);
...
void use_persistent_comm(MPI_Datatype* dtype, int count, int size, int num_iters) {
    int rank, numtasks;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &numtasks);
...
int main(int argc, char **argv) {
    ...
    if(rank == 0) printf("ONE SIDED PERSISTENT:\ndata %d, strategy %d, iterations %d, count %d and size %d: %lfs\n\n", 

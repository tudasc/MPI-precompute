...
void init_forward_communication(int rank, int size)
{
    int other = MPI_PROC_NULL;
    if(rank != 0){
        other = rank-1;
    }
    MPI_Send_init(&DATA[25], 25, MPI_DOUBLE, other, tag_to_use, MPI_COMM_WORLD, &comm_requests[0]);
    MPI_Recv_init(&DATA[0], 25, MPI_DOUBLE, other, tag_to_use, MPI_COMM_WORLD, &comm_requests[1]);
  
}

void init_backward_communication(int rank, int size)
{
    int other = MPI_PROC_NULL;
    if (rank != size-1) {
        other = rank+1;
    }
    MPI_Send_init(&DATA[75], 25, MPI_DOUBLE, other, tag_to_use, MPI_COMM_WORLD, &comm_requests[2]);
    MPI_Recv_init(&DATA[50], 25, MPI_DOUBLE, other, tag_to_use, MPI_COMM_WORLD, &comm_requests[3]);

}

void init_communication(int rank, int size){
    
    init_forward_communication(rank,  size);
    init_backward_communication( rank,  size);
    tag_to_use++;
